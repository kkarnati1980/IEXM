...
import { createIotIncidentSyncService } from "../iot/incident-sync-service.mjs";
...
try {
  ...
  const service = createIotIncidentSyncService({
    adapter,
    repos: app.repos,
    pageLimit: Number(process.env.IOT_INCIDENT_PAGE_LIMIT ?? 100)
  });
