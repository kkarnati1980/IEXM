...
import { createIotHeartbeatSyncService } from "../iot/heartbeat-sync-service.mjs";
...
try {
  ...
  const service = createIotHeartbeatSyncService({
    adapter,
    repos: app.repos,
    pageLimit: Number(process.env.IOT_HEARTBEAT_PAGE_LIMIT ?? 100)
  });
